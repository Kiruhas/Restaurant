...
namespace KursachTest
{
    ...
    public class Production // Объект продукт со складом
    {
        public string name;
        public int count;
        public int sklad;
    }
    public class Sklad // Объект склад
    {
        public int number;
        public int temp;
    }
    public class DishWithComponent // Объект блюдо с составом
    {
        public string name;
        public int price;
        public int weight;
    ...
    class Functions
    {
        ...
        public static void GetDishComponent(string dishName, List<Product> prod) // Запрос состава блюда
        {
            ...
            NpgsqlCommand command = new NpgsqlCommand("SELECT ProductName, countprod FROM " +
                "public.dishComponent WHERE DishName = '" + dishName + "'", connect);
            ...
            using (command)
            {
                ...
                while (reader.Read())
                {
                    ...
                    countProd = Convert.ToInt32(reader[1]);
                    Product item = new Product();
                    item.name = nameProd;
                    item.count = Convert.ToDouble(countProd);
                    prod.Add(item);
                }
            }
        ...
        public static void GetProducts(List<string> products) // Получение списка продуктов
        {
            connect.Open();
        ...
        public static void GetProductions(List<Production> product) // Получение списка продуктов со складом
        {
            connect = new NpgsqlConnection(connString);
            connect.Open();
        ...
        /*BUTTONS ON_CLICK*/
        private static void button_Click_Order(object sender, EventArgs e) // функция для кнопок заказов
        {
        ...
        private static void button_Click(object sender, EventArgs e) // функция для кнопок блюд
        {
            var button = (Button)sender;
            if (button != null)
            {
        ...
        /*BUTTONS ON_CLICK---END*/


        /*ADD, UPDATE, DELETE - DISH, PRODUCTS, COMPONENTS*/
        public static void AddDish(string name, double price, double weight, string category, List<Product> productList) // Добавить блюдо в базу
        {
            connect = new NpgsqlConnection(connString);
        ...
        public static void DeleteDishWithComponent(string name) // Удалить блюдо с составом
        {
            connect = new NpgsqlConnection(connString);
        ...
        public static void AddProduct(string name, int sklad, int count) // Добавить продукт в базу
        {
            connect = new NpgsqlConnection(connString);
        ...
        public static void DeleteProduct(string name, int sklad)  // Удалить продукт из базы
        {
            connect = new NpgsqlConnection(connString);
        ...
        public static void UpdateProducts(List<string> prod) // Обновление количества продуктов на складе
        {
            connect = new NpgsqlConnection(connString);
        ...
        public static void RemoveProducts(List<Dish> dishList) // Обновить количество продуктов в базе
        {
            for (int i = 0; i < dishList.Count; i++)
            {
                ...
                GetDishComponent(nameDish, containProduct);

                for (int j = 0; j < containProduct.Count; j++)
                {
                    int countProdInDish = dishList[i].count * Convert.ToInt32(containProduct[j].count);
                    connect = new NpgsqlConnection(connString);
                    using (connect)
                    {
                        connect.Open();
                        NpgsqlCommand command = new NpgsqlCommand("UPDATE public.product" +
                            " SET quantity = quantity - " + countProdInDish +
                            " WHERE productname = '" + containProduct[j].name + "'", connect);
                        command.ExecuteNonQuery();
                    }
                }
            }
        ...
        public static void GetSklad(List<Sklad> skladList) // Получение списка складов
        {
            skladList.Clear();
        ...
        public static void AddSklad(string number, string temp) // Добавление склада
        {
            connect = new NpgsqlConnection(connString);
        ...
        public static void AddStaff(string fio, string dolzhn, string stazh, string login, string pass) // добавление сотрудника
        {
            connect = new NpgsqlConnection(connString);
        ...
        public static void AddNewClient(string name, string number, int age, int sale) // Добавить карту клиента
        {
            connect = new NpgsqlConnection(connString);
        ...
        /*ADD, UPDATE, DELETE - DISH, PRODUCTS, COMPONENTS---END*/


        /*WORK WITH LOGIN*/
        public static string GetHash(string plaintext) // получение хэша пароля
        {
            var sha = new SHA1Managed();
            byte[] hash = sha.ComputeHash(Encoding.UTF8.GetBytes(plaintext));
            return Convert.ToBase64String(hash);
        }
        public static string CheckStaff(string login, string pass) // Проверка на существование сотрудника
        {
            connect = new NpgsqlConnection(connString);
            connect.Open();
        ...
        public static string ReturnFio(string login) // Возвращает фамилию сотрудника, который зашел в приложение
        {
            string fio = "";
            connect = new NpgsqlConnection(connString);
            connect.Open();
